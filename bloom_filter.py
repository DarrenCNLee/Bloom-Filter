...
class BloomFilter:
    ...
    def add(self, x):
        ...
        # loop for k hash functions
        for i in range(self.k):

            # set the bit at row i and column hash(x,i) in the bit array to true
    ...
    def contains(self, x) -> bool:
        ...
        # check that none of the bits are set to false in the list of bits at rows i
        # and columns hash(x,i)
        return False not in [self.t[i][self.hash(x,i)] for i in range(self.k)] 
